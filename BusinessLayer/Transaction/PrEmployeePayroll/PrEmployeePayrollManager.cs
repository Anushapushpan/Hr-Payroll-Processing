...
namespace BusinessLayer.Transaction.PrEmployeePayroll
{
    public class PrEmployeePayrollManager
    {
        ...
        public DataTable CheckPayroll(string yyyymm)
        {
            try
            {
                DataTable gd = new DataTable();
                gd = DBConnection.ExecuteDataset($"SELECT COUNT(*) COUNT FROM PR_EMPLOYEE_PAYROLL WHERE PR_YYYMM='{yyyymm}'");
                return gd;
            }
            catch (Exception ex)
            {
        ...
        public DataTable FetchEmpNo(string yyyymm)
        {
            try
            {
                string query = $"SELECT ATT_EMP_NO FROM PR_EMPLOYEE_ATTENDANCE WHERE ATT_YYYYMM='{yyyymm}'";
